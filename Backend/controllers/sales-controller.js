...
const updateSale = async (req, res) => {
  try {
    ...
    if (newQty < 1) return res.status(400).json({ message: 'quantity must be >= 1' });

    // handle inventory adjustments
    const prevProductId = String(sale.productId);
    const prevQty = Number(sale.quantity) || 0;

    // if product changed, restore previous product stock and reduce new product
    if (prevProductId !== String(productId)) {
      const prevProduct = await Product.findById(prevProductId);
      if (prevProduct && typeof prevProduct.quantity === 'number') {
        prevProduct.quantity = prevProduct.quantity + prevQty;
        await prevProduct.save();
      }

      const newProduct = await Product.findById(productId);
      if (!newProduct) return res.status(400).json({ message: 'Invalid productId' });
      if (typeof newProduct.quantity === 'number' && newProduct.quantity < newQty) {
        return res.status(400).json({ message: 'Insufficient stock for selected product' });
      }
      // decrement new product
      if (typeof newProduct.quantity === 'number') {
        newProduct.quantity = newProduct.quantity - newQty;
        if (newProduct.quantity < 0) newProduct.quantity = 0;
        await newProduct.save();
      }

      sale.productId = newProduct._id;
      sale.productName = newProduct.title || newProduct.name || newProduct.productName || String(newProduct._id);
      sale.quantity = newQty;
      sale.total = Number(newProduct.price || 0) * newQty;
    } else {
      ...
      if (diff > 0) {
        if (typeof product.quantity === 'number' && product.quantity < diff) {
          return res.status(400).json({ message: 'Insufficient stock for selected product' });
        }
        if (typeof product.quantity === 'number') {
      ...
      sale.quantity = newQty;
      sale.total = Number(product.price || 0) * newQty;
    }

    sale.customerName = String(customerName).trim();
    if (status !== undefined) sale.status = status;
    if (createdAt) sale.createdAt = new Date(createdAt);
...
module.exports = { listSales, createSale, deleteSale, updateSale };